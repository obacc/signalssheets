...
class BigQuerySetup:
    ...
    def run_block_a0_verifications(self):
        """Block A0: Pre-flight checks (corrected for actual schema)."""
        print("\n" + "="*80)
        print("📋 BLOCK A0 — PRE-VERIFICATION CHECKS (Schema Corrected)")
        print("="*80)
        ...
        SELECT
          ...
          COUNT(*) AS total_filas,
          COUNT(DISTINCT ticker) AS tickers_unicos,
          COUNT(CASE WHEN name IS NULL THEN 1 END) AS nombres_nulos
        ...
        WITH
        max_dates AS (
          SELECT 'Prices' AS fuente, MAX(fecha) AS max_date
          FROM `{self.project_id}.market_data.Prices`
        )
        SELECT
          fuente,
    ...
    def run_block_b_product_view(self):
        """Block B: Create main product view (corrected for actual schema)."""
        print("\n" + "="*80)
        print("📦 BLOCK B — MAIN PRODUCT VIEW (Schema Corrected)")
        print("="*80)

        query_b = f"""
        CREATE OR REPLACE VIEW `{self.project_id}.analytics.v_api_free_signals` AS
        WITH
        -- Subconsulta 1: Top 10 del ranking actual (canonizado)
        t10 AS (
          SELECT
        ...
        -- Subconsulta 2: Último precio por ticker (optimizado)
        lp AS (
          SELECT
          ...
          WHERE pc.ticker_canon IN (SELECT DISTINCT ticker FROM t10)
            AND pc.fecha >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)
          GROUP BY pc.ticker_canon
        ),
        -- Subconsulta 3: Fecha de señal EOD (si existe)
        sig_dates AS (
          SELECT
        ...
        -- Subconsulta 4: Mapeo de sectores (ya canonizado en la tabla)
        sector_map AS (
          SELECT DISTINCT
            CONCAT(m.ticker_canon, '.US') AS ticker,
            COALESCE(m.sector, 'Unknown') AS sector
          FROM `{self.project_id}.analytics.sector_map_v6r2` m
          WHERE CONCAT(m.ticker_canon, '.US') IN (SELECT DISTINCT ticker FROM t10)
        ),
        -- Subconsulta 5: Nombres de empresas (usando columna 'name')
        company_names AS (
          SELECT DISTINCT
        ...
        SELECT
          ...
          -- Fechas
          COALESCE(sd.signal_date, CURRENT_DATE()) AS signal_date,
    ...
    def run_block_c_status_view(self):
        """Block C: Create status view (corrected for actual schema)."""
        print("\n" + "="*80)
        print("📊 BLOCK C — STATUS VIEW (Schema Corrected)")
        print("="*80)
        ...
        lags AS (
          SELECT
            (SELECT max_date FROM max_price_date) AS as_of_date,
            (SELECT COUNT(*)
             FROM cal
             WHERE cal_date > (SELECT max_date FROM max_price_date)
               AND cal_date <= CURRENT_DATE()
            ) AS lag_td
        )
        SELECT
          CURRENT_TIMESTAMP() AS generated_at_utc,
          (SELECT as_of_date FROM lags) AS as_of_date,
          (SELECT lag_td FROM lags) AS lag_trading_days,
          (SELECT n FROM signal_data) AS row_count,
          (SELECT n FROM signal_data_with_price) AS rows_with_valid_price,

          -- Criterio FREE: corte ≤1 TD, al menos 5 filas con precio
          (
            (SELECT lag_td FROM lags) <= 1
            AND (SELECT n FROM signal_data_with_price) >= 5
          ) AS api_signals_ready,

          -- Metadata adicional
          CASE
            WHEN (SELECT lag_td FROM lags) > 1 THEN 'Data stale - update required'
            WHEN (SELECT n FROM signal_data_with_price) < 5 THEN 'Insufficient signals with prices'
            ELSE 'Ready to serve'
    ...
    def run_block_d_validations(self):
        ...
        SELECT
          ...
          COUNT(CASE WHEN sector IN ('Unknown', 'Uncategorized') THEN 1 END) AS sin_sector,
        ...
        SELECT
          ...
          (SELECT COUNT(*) FROM base b
           WHERE EXISTS(
             ...
             WHERE CONCAT(s.ticker_canon, '.US') = b.ticker
           )
          ) AS con_sector
        """
    ...
    def create_documentation(self):
        ...
        # Delete existing docs for this endpoint
        query_delete = f"""
        DELETE FROM `{self.project_id}.analytics.api_endpoint_docs`
        WHERE endpoint = '/api/free/signals'
        """
        self.execute_query(query_delete, "Delete existing documentation")
...
def main():
    ...
    print("\n" + "="*80)
    print("🚀 BigQuery FREE Signals Endpoint Setup - V2 (Schema Corrected)")
    print("="*80)
